#ifndef __PSRD_NOISE_2D__
#define __PSRD_NOISE_2D__
...
// Map from a line to a diamond such that a shift maps to a rotation.
  ...
  u = 4.0 * frac(u) - 2.0;
  // (This floattor could be normalized, exactly or approximately.)
...
float3 psrdnoise(float2 pos, float2 per, float rot) {
  ...
  // floattors in unskewed (x,y) coordinates from
  ...
  // Gradients dot floattors to corresponding corners
  ...
  // sum of ((radial weights) times (gradient dot floattor from corner))
    float n = dot(t4, w);
