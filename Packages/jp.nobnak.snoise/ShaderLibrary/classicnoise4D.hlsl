...
[noinline] float cnoise(float4 P) {
...
[noinline] float pnoise(float4 P, float4 rep) {
...
[noinline] void cnoise_float(float4 In, out float Out) {
    Out = cnoise(In);
}
[noinline] void pnoise_float(float4 In, float4 Rep, out float Out) {
    Out = pnoise(In, Rep);
}
